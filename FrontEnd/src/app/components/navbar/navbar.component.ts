...
export class NavbarComponent implements OnInit {
  ...
  // total: any;
  // totalFavRestaurants: number = 0;
  // totalFavCuisines: number = 0;
  ...
  checkLogin() {
    ...
    if (window.localStorage.getItem('userEmail') != null) {
      // this.favourite.getFavRestaurant()
      //   .subscribe(res => {
      //     this.totalFavRestaurants = res.length;
          this.getImage();
        // })
      // this.favourite.getFavCuisine()
      //   .subscribe(res => {
      //     this.totalFavCuisines = res.length;
      //     this.total = this.totalFavRestaurants + this.totalFavCuisines;
      //   })
  ...
  saveProfile(event: any) {
    if (event.target.files) {
      ...
      // reader.onload = (profileEvent : any) =>
      // {
      //   this.profileUrl = profileEvent.target.result;
      // }
    }
  }
  ...
  onUpload() {
    ...
    this.http.post<any>('http://localhost:9090/foodieApp/profile/upload', fd)
      .subscribe(res => {
        console.log(res);
  ...
  getImage() {
    this.http.get<any>('http://localhost:9090/foodieApp/profile/image/' + this.userEmail)
      .subscribe(res => {
        // console.log(res)
        // console.log(res.url);
  ...
  logoutUser() {
    ...
    this.toast.success({ detail: 'Logout successfull', duration: 5000 });
