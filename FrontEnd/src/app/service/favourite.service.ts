...
//This method will add favourite restautants to the cart
  ...
  {
    const restaurantUrl = 'http://localhost:9090/foodieApp/favourites/addFavRestaurant';
  ...
  {
    const cuisineUrl = 'http://localhost:9090/foodieApp/favourites/addFavCuisine';
    if(window.localStorage.getItem('tokenKey') == null)
    {
      this.toast.warning({detail:'please login to add favourites',duration:2000});
    }
  ...
  {
    const favRestautantUrl = 'http://localhost:9090/foodieApp/favourites/favRestaurantByEmailId/';
  ...
  {
    const favCuisineUrl = 'http://localhost:9090/foodieApp/favourites/favCuisineByEmailId/';
  ...
  removeFavRestaurant(restaurantId: any) {
    const deleteUrl = 'http://localhost:9090/foodieApp/favourites/delRestaurant/' + restaurantId;
  ...
  removeFavCuisine(cuisineId: any) {
    const deleteUrl = 'http://localhost:9090/foodieApp/favourites/favCuisine/' + cuisineId;
    return this.http.delete<any>(deleteUrl,
      { headers: this.getHeader(window.localStorage.getItem('tokenKey')) })
      .subscribe( res =>{
